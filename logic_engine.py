...
def run_level50(spreadsheet_id, sheet_name="RFQ TEST SHEET", debug=False):
    ...
    # 🚨 HARD BYPASS: AI ko call hi mat karo agar "Short substrate" ka lafda hai
    trace_id = f"L80-{datetime.now().strftime('%d%H%M%S')}"
    
    print(f"🚀 LEVEL-80 SYSTEM STARTING | Trace: {trace_id} | BYPASS MODE ON")
    sys.stdout.flush()
