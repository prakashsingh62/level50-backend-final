...
from sheet_writer import apply_updates
...
async def test_write_api():
    try:
        apply_updates([{"row": 2, "column": "TEST_COL", "value": "API_OK"}])
...
async def run_api(debug: bool = False):
    """
    debug = true  → ALWAYS SEND EMAIL even if no sections generated.
    debug = false → Production mode (if nothing to send, skip email).
    """
    ...
    try:
        ...
        if debug:
            if not email_content.strip():
                email_content = (
                    "<h3>[DEBUG MODE]</h3>"
                    "<p>No reminder sections were generated by logic_engine.py.</p>"
                    "<p>This email is sent only for testing email delivery.</p>"
                )

        # If still empty AND not debug → skip email sending
        if not email_content.strip():
